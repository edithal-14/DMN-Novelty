...
from spacy_decomposable_attention import (
    _Attention,
    _SoftAlignment,
    _Comparison,
    _Entailment
...
#     golds = [all_golds[idx] for idx in ids]
    ...
    LOG.debug("Number of instances: %s", len(src_ids))
    ...
    attention = _Attention(max_sents, SENT_DIM, dropout=0.2)(tgt_emb, srcs_emb)
    align1 = _SoftAlignment(max_sents, SENT_DIM)(srcs_emb, attention)
    align2 = _SoftAlignment(max_sents, SENT_DIM)(tgt_emb, attention, transpose=True)
    vec_l = _Comparison(max_sents, SENT_DIM, dropout=0.2)(tgt_emb, align1)
    vec_r = _Comparison(max_sents, SENT_DIM, dropout=0.2)(srcs_emb, align2)
    ...
    model.fit(x=[tgt_ids[train],src_ids[train]],y=golds[train],batch_size=BATCH_SIZE,nb_epoch=NUM_EPOCHS,shuffle=True,verbose=1)
...
init_logger(LOG, 'ste_decom_attn.log')
