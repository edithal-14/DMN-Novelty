...
from apwsj_logger import init_logger
...
class DMNPlus(nn.Module):
    ...
    def forward(self, contexts, questions):
        '''
        contexts.size() -> (#batch, #context, #embedding) -> (#batch, #context, #hidden)
        questions.size() -> (#batch, #sentence, #embedding) -> (1, #batch, #hidden)
        '''
...
if __name__ == "__main__":
    ...
    model = DMNPlus(hidden_size, num_hop=3)
    model.cuda()
    ...
    with open(f'models/epoch{epoch}_acc{best_acc}.pth', 'wb') as fp:
